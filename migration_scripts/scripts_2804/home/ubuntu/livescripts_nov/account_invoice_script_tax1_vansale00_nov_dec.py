...
def get_cogs_price_updates(v18_rec,v12_rec,cogs_mapped_idv18,stock_outputv18,cogs_mapped_idv12,stock_outputv12,acc_recvbl18,acc_recbl12,acc_payble18,acc_payble12,p_id18,p_id12,amlv12,vansal_tax_act12,vansal_tax_act18,rpl_tax):
    ...
    #print("111111111 >>>>>>>>>>>>>",rec18_cog,ac_recv12_cogs)
    #print("22222222222222 >>>>>>>>>>>>>",rec18_stock,rec18_stock)
    if ac_recv12_cogs:
        counter = 0
        for v18l in rec18_cog:
            ...
            print(sa1)
    elif rec18_cog and not ac_recv12_cogs:
        c_id = rec18_cog[0]['id']
        sql_delete2 = f"delete from account_move_line where id={c_id}"
        san2 = sock18.execute_kw(dbname18, uid18, pwd18, 'account.move', 'execute_raw_sql', [sql_delete2])
        print(san2)
    ...
    if ac_recv12_stock:
        ...
        for v18l2 in rec18_stock:
            ...
            print(sa2)
    elif rec18_stock and not ac_recv12_stock:
        c1_id = rec18_stock[0]['id']
        sql_delete3 = f"delete from account_move_line where id={c1_id}"
        san3 = sock18.execute_kw(dbname18, uid18, pwd18, 'account.move', 'execute_raw_sql', [sql_delete3])
        print(san3)
...
# 165376,165288, 172128,172030,165012,171925, 164882,171821,164867,171714,164787
#domain = [('id','in',[164867,171714,164787])]
account_invoice = sock.execute_kw(dbname, uid, pwd, 'account.invoice', 'search', [[('type','!=','out_refund'),('state','not in',['draft','cancel']),('date_invoice','>=','2024-11-01'),('date_invoice','<=','2024-11-30'),('journal_id','=',266),('company_id','=',2)]],{'order': 'id DESC'})

#account_invoice = sock.execute_kw(dbname, uid, pwd, 'account.invoice', 'search', [domain],{'order': 'id DESC'})
# account_invoice = sock.execute_kw(dbname, uid, pwd, 'account.invoice', 'search', [[('type','!=','out_refund'),('date_invoice','=','2024-01-01'),('company_id','=',2),('journal_id','=',266)]],{'order': 'id DESC'})
...
####################
account_invoice = account_invoice[100:]
    ...
    for rec in range(0,len(account_invoice)):
        ...
        if not rec_is_exitst:
            ...
            if state18 in ['open','in_payment','paid']:
                ...
                acc_payble12 = 243
                vansal_tax_act12 = 273
 
                rpl_tax = 766 # Tota sale account to chaged because details chaged,Modification applied
