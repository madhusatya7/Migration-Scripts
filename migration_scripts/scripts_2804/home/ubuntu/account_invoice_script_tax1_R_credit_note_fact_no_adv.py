#######Reversal vansales that advanced option levelve lines are deleted COGS, we need give give duplicate pproducts exctactly same COGS
...
def get_cogs_price_updates(v18_rec,v12_rec,cogs_mapped_idv18,stock_outputv18,cogs_mapped_idv12,stock_outputv12,acc_recvbl18,acc_recbl12,acc_payble18,acc_payble12,p_id18,p_id12,amlv12,vansal_tax_act12,vansal_tax_act18,rpl_tax,c):
    # print("\n\n>>>>>>>>>>>>>??",cogs_mapped_idv18,stock_outputv18,c)
    # already created as per current cogs and cost current DB main company
    rec18_cog = sock18.execute_kw(dbname18, uid18, pwd18, 'account.move.line','search_read', [[('move_id','=',v18_rec),('account_id', '=',75),('display_type','=','cogs'),('product_id','=',p_id18)]], {'fields': ['id','old_id','account_id','partner_id','product_id','name','debit','credit']})
    rec18_stock = sock18.execute_kw(dbname18, uid18, pwd18, 'account.move.line','search_read', [[('move_id','=',v18_rec),('account_id', '=',169),('display_type','=','cogs'),('product_id','=',p_id18)]], {'fields': ['id','old_id','account_id','partner_id','product_id','name','debit','credit']})
    ac_receivabl8 = sock18.execute_kw(dbname18, uid18, pwd18, 'account.move.line','search_read', [[('move_id','=',v18_rec),('account_id', '=',10),('display_type','=','payment_term'),('product_id','=',False)]], {'fields': ['id','old_id','account_id','partner_id','product_id','name','debit','credit']})
    ac_payvabl8 = sock18.execute_kw(dbname18, uid18, pwd18, 'account.move.line','search_read', [[('move_id','=',v18_rec),('account_id', '=',53),('display_type','=','payment_term'),('product_id','=',False)]], {'fields': ['id','old_id','account_id','partner_id','product_id','name','debit','credit']})
    ac_tax_line18 = sock18.execute_kw(dbname18, uid18, pwd18, 'account.move.line','search_read', [[('move_id','=',v18_rec),('account_id', '=',vansal_tax_act18),('display_type','=','tax'),('product_id','=',False)]], {'fields': ['id','old_id','account_id','partner_id','product_id','name','debit','credit']})
    ...
    #
    # print("rec18_cog >>>>>>>>",rec18_cog)
    # for v18l in rec18_cog:
    # print("counter >>>>>>>>>>>>>",c,ac_recv12_cogs)
    # print("Targeted data >>>>>>>>>>>>>",c,ac_recv12_cogs[c])
    
    aml_id = rec18_cog[0]['id']

    amt_d = ac_recv12_cogs[c]['debit']
    amt_c = ac_recv12_cogs[c]['credit']
    ...
    sql_cogs = f"update account_move_line set credit={amt_c},balance={amount_currency18},account_id={cogs_mapped_idv18} where id={aml_id}"
    # print(sql_cogs)
    ...
    # print(">>>>>>>>>>>>>>>rec18_stock",rec18_stock)
    aml_id2 = rec18_stock[0]['id']
    amt2_c = ac_recv12_stock[c]['credit'] 
    amt2_d = ac_recv12_stock[c]['debit']
    ...
    # counter1 += 1
    sql_stck_op = f"update account_move_line set debit={amt2_d},balance={amount_currency118},account_id={stock_outputv18} where id={aml_id2}"
...
# domain = [106546,106377,105422,105222,105089,113291,104707,104300,104198,103384,101992,101959,101429,106833,106730,106641,100668,99726,126799,125656]
#domain = [('id','in',[101062,102857,109318,110044,110293,112508,116985])]
domain = [('id','in',[118764,119444,123478,123706,124370,129808,131463,132740,137315,137536,137981,138368,139897,141188,141749,142292,144700,144702,145160,145219,146869,147040,147231,148684,151056,152464,152744,154016,154023,154076,158497,160721,161988,164071,167866,170179,170190,170195,171350,172525])]
# domain = [('journal_id','=', 266),('type','=','out_refund'),('date_invoice','>=','2024-01-01'),('date_invoice','<=','2024-03-26'),('company_id','=',2)]

account_invoice = sock.execute_kw(dbname, uid, pwd, 'account.invoice', 'search', [domain],{'order': 'id DESC'})
# account_invoice = sock.execute_kw(dbname, uid, pwd, 'account.invoice', 'search', [[('journal_id','in',[10,262]),('type','=','out_refund'),('date_invoice','>=','2024-01-01'),('date_invoice','<=','2024-01-02'),('company_id','=',2)]],{'order': 'id DESC'})


print("length",len(account_invoice))

####################
#account_invoice = account_invoice[15000:]
    ...
    for rec in range(0,len(account_invoice)):
        ...
        if not rec_is_exitst:
            ...
            if state18 in ['open','in_payment','paid']:
                ...
                acc_payble12 = 243
                vansal_tax_act12 = 229

                rpl_tax = 622 # Tota sale account to chaged because details chaged,Modification applied
                ...
                ##to catcht the details of the details of repeat itmes and no account advanced option and cost of good sold having to show difftent cost of repeated cost
                result9 = []
                counts = {}

                for item in prod_list:
                    # Use only the first two elements of the tuple as the key for counting
                    key = item[:2]
                    ...
                    # Append the tuple with the count as a new element
                    result9.append((*item, counts[key]))
                ...
                for p in result9:
                    ...
                    amlv12 = p[2]
                    c=p[3]
                    rn = get_cogs_price_updates(reslt,rec12_id,cogs_mapped_idv18,stock_outputv18,cogs_mapped_idv12,stock_outputv12,acc_recvbl18,acc_recbl12,acc_payble18,acc_payble12,p_id18,p_id12,amlv12,vansal_tax_act12,vansal_tax_act18,rpl_tax,c)
