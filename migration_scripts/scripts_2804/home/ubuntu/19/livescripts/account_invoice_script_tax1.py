...
def get_cogs_price_updates(v18_rec,v12_rec,cogs_mapped_idv18,stock_outputv18,cogs_mapped_idv12,stock_outputv12,acc_recvbl18,acc_recbl12,p_id18,p_id12,amlv12):
    # recds = []
    ...
    # ac_receivable12 = sock.execute_kw(dbname, uid, pwd, 'account.move.line','search_read', [[('move_id','=',v12_rec),('account_id', '=',acc_recbl12),('product_id','=',p_id12)]], {'fields': ['id','account_id','partner_id','product_id','name','debit','credit']})
    ...
    for v18l in rec18_cog:
        ...
        counter += 1 
        # print (">>>>>>>>>>v18l",v18l,amt,cogs_mapped_idv18)
        ...
        # print("??>>>>",sql_cogs)
        sa = sock18.execute_kw(dbname18, uid18, pwd18, 'account.move', 'execute_raw_sql', [sql_cogs])
    ...
    for v18l2 in rec18_stock:
        ...
        #print("\n\ncounter1,ac_recv12_stock>>>>>>>>",counter1,ac_recv12_stock)
        ...
        #print("??>>>sql_stck_opsql_stck_opsql_stck_op>",sql_stck_op)
    ...
    for v18l3 in ac_receivabl8:
        ...
        # print(">>>>>>>>>>>",ac_receivabl8)
        # print(counter3)
        # amt3 = ac_receivable12[0]['debit']   
        counter3 += 1
        sql_account_recb = f"update account_move_line set account_id={acc_recvbl18} where id={aml_id3}"
        # print("??>>>sql_account_recb>",sql_account_recb)
        sa1 = sock18.execute_kw(dbname18, uid18, pwd18, 'account.move', 'execute_raw_sql', [sql_account_recb])
    return True
...
#account_invoice = sock.execute_kw(dbname, uid, pwd, 'account.invoice', 'search', [[('date_invoice','>=','2024-01-01'),('company_id','=',6)]],{'order': 'id DESC'})
...
#account_invoice = sock.execute_kw(dbname, uid, pwd, 'account.invoice', 'search', [[('date_invoice','>=','2024-01-01'),('date_invoice','<=','2024-01-31'),('company_id','=',2)]],{'order': 'id DESC'})

#account_invoice = sock.execute_kw(dbname, uid, pwd, 'account.invoice', 'search', [[('id','=',114336),('company_id','=',2)]],{'order': 'id DESC'})
    ...
    for rec in range(0,len(account_invoice)):
        ...
        if not rec_is_exitst:
            ...
            if state18 in ['open','in_payment','paid']:
                ...
                # print("prod_ids is>>>>>>>",prod_id)
                cogs_mapped_idv18 = 994 #v18
                stock_outputv18 = 933 #v18
                acc_recvbl18 = 788 #v18
                ...
                for p in prd:
                    ...
                    # print ('>>>>>>>>>>>p_id',p_id18,p_id12)
                    rn = get_cogs_price_updates(reslt,rec12_id,cogs_mapped_idv18,stock_outputv18,cogs_mapped_idv12,stock_outputv12,acc_recvbl18,acc_recbl12,p_id18,p_id12,amlv12)
