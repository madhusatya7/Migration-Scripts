...
#direct 115204,8787
domain = [('id','=',4672)]
# domain = [('journal_id','=',9),('company_id','=',2),('state','not in',['draft','cancel'])]
...
####################
#account_invoice = account_invoice[950:]
...
if len(account_invoice)>0:
    for rec in range(0,len(account_invoice)):
        account_invoice_rec = sock.execute_kw(dbname, uid, pwd, 'account.invoice', 'search_read',[[('id', '=', account_invoice[rec])]], {
												'fields': ['name','number','date_invoice','date','date_due','partner_id','partner_shipping_id','reference',
                                                           'payment_term_id','user_id','currency_id','journal_id','company_id','type','state','job_number',
                                                           'bl_number','container_no','account_analytic_id','partner_bank_id','comment']})
        ...
        reference18 = account_invoice_rec[0]['reference'] if account_invoice_rec[0]['reference'] else ''
        comment18 = account_invoice_rec[0]['comment']
        ...
        create_vals = {'old_id':record_ID,'name':number18,'invoice_date':date_invoice18,'date': date18 or dt.now().date().strftime("%Y-%m-%d") ,'invoice_date_due':date_due18,'partner_id':partner_id18,
                       'currency_id':currency_id18,'journal_id':journal_id18,'company_id':company_id18,'bl_number':bl_number18,'container_no':container_no18,'ref':mname18,'payment_reference':reference18,
                       'move_type':move_type18,'invoice_user_id':user_id18, 'partner_bank_id':partner_bank_id18,'job_number':job_number18,'invoice_payment_term_id':invoice_payment_term_id18,'narration':comment18} #'partner_shipping_id':partner_shipping_id18

        account_invoice_rec_line = sock.execute_kw(dbname, uid, pwd, 'account.invoice.line', 'search_read',
                                              [[('invoice_id', '=', record_ID)]], {
                                                  'fields': ['name', 'product_id','account_id', 'quantity', 'uom_id','price_unit', 'invoice_line_tax_ids', 'discount', 'price_subtotal','account_analytic_id']})
        ...
        for line in account_invoice_rec_line:
            ...
            if not product_id:
                ...
                account_id = line['account_id'][0] if line['account_id'] else False
                ...
                obj = {'name':name18,'product_id':product_id18,'quantity':quantity18,'partner_id':partner_id18,
                       'product_uom_id':uom_id18,'price_unit':price_unit18, 'tax_ids':invoice_line_tax_ids18,
                       'discount':discount18, 'price_subtotal':price_subtotal18,'account_id':account_id18,
