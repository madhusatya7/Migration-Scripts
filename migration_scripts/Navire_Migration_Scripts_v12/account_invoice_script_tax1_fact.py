...
# version 18
url_common18="http://localhost:8019/xmlrpc/2/common"
url_object18="http://localhost:8019/xmlrpc/2/object"
...
if len(account_invoice)>0:
    for rec in range(0,len(account_invoice)):
        account_invoice_rec = sock.execute_kw(dbname, uid, pwd, 'account.invoice', 'search_read',[[('id', '=', account_invoice[rec])]], {
												'fields': ['name','number','date_invoice','date','date_due','reference','partner_id',
                                                           'partner_shipping_id','payment_term_id','user_id','currency_id','journal_id','company_id','type','state',
                                                           'job_number','bl_number','container_no','account_analytic_id']})
        ...
        # job_number18 = account_invoice_rec[0]['job_number']
        ...
        create_vals = {'old_id':record_ID,'name':number18,'invoice_date':date_invoice18,'date': date18 or dt.now().date().strftime("%Y-%m-%d") ,'invoice_date_due':date_due18,'payment_date': date_invoice18 or dt.now().date(),'payment_reference':reference18,'partner_id':partner_id18,'currency_id':currency_id18,'journal_id':journal_id18,'company_id':company_id18,
                       'move_type':move_type18,} #'job_number':job_number18,'bl_number':bl_number18,'container_no':container_no18,'partner_shipping_id':partner_shipping_id18
        ...
        for line in account_invoice_rec_line:
            ...
            # account_analytic_id18 = get_mapped_rec(model='account.analytic.account',ID=account_analytic_id) if account_analytic_id else False
            ...
            obj = {'old_id':aml_id,'name':name18, 'display_type': 'product','product_id':product_id18, 'account_id':account_id18,'quantity':quantity18, 'product_uom_id':uom_id18,'price_unit':price_unit18, 'tax_ids':invoice_line_tax_ids18, 'discount':discount18, 'price_subtotal':price_subtotal18} #'analytic_account_id':account_analytic_id18
