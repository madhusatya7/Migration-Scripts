    ...
    for rec in range(0,len(sale_order)):
        res_rec = sock.execute_kw(dbname, uid, pwd, 'sale.order', 'search_read',[[('id', '=', sale_order[rec])]], {
												'fields': ['name','partner_id','payment_term_id','confirmation_date','job_type','job_created_by','picking_policy','user_id','tag_ids','team_id','invoice_status','invoice_ids',
                                                           'pricelist_id','client_order_ref','bl_no','company_id','analytic_account_id','origin','opportunity_id','campaign_id','medium_id','source_id','state','note']})
        ...
        create_vals = {'old_id':record_ID,'name':name18,'date_order':confirmation_date18,'picking_policy':picking_policy18,'client_order_ref':client_order_ref18,'bl_no':bl_no18,
                       ...
                       'job_created_by':job_created_by18,'note':note18}
