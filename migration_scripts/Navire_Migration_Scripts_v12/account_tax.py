...
if len(account_tax)>0:
    for rec in range(0,len(account_tax)):
        ...
        # name wise unique
        ac_rec_is_exist = sock18.execute_kw(dbname18, uid18, pwd18, 'account.tax', 'search_read',
                                            [[('name', '=', name18)]], {'fields': ['id', 'old_id', 'type_tax_use']})
        ...
        # company wise unique
        ac_rec_cmp_is_exist = sock18.execute_kw(dbname18, uid18, pwd18, 'account.tax', 'search_read',
                                                [[('name', '=', name18), ('company_id', '=', company_id18)]],
                                                {'fields': ['id', 'old_id', 'type_tax_use', 'company_id']})
        if not ac_rec_cmp_is_exist:
            create_vals['name'] = str(company_id18) + "-" + str(create_vals['name'])
            result = sock18.execute_kw(dbname18, uid18, pwd18, 'account.tax', 'create', [create_vals])
        else:
            # for type also
            duplicate_is_exist = sock18.execute_kw(dbname18, uid18, pwd18, 'account.tax', 'search_read', [
                [('type_tax_use', '=', type_tax_use18), ('name', '=', name18), ('company_id', '=', company_id18)]],
                                                   {'fields': ['id', 'old_id', 'type_tax_use']})
            if not duplicate_is_exist:
                create_vals['name'] = str(company_id18) + "-" + str(create_vals['name']) + "[" + str(
                    type_tax_use18) + "]"
                result = sock18.execute_kw(dbname18, uid18, pwd18, 'account.tax', 'create', [create_vals])
            else:
                print(f"\n Record existed?\n")
